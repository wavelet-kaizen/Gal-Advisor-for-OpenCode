...
```text
RUNNING → REQUIRED → CONSULTING → CONTRACT → NEXT → RUNNING
...
4. 主エージェントは `gal_recover` にACCEPT/REJECT、理由、証拠ID、次のtool/argsを渡します。
   ACCEPTはAdvisorの指定と一致する必要があります。REJECTは異なる操作が必要です。
5. 指定した1操作の完了後に通常作業へ戻ります。契約中の並列操作は拒否します。

同じ問題の相談は最大2回です。前回と観測証拠が同じなら再相談を拒否しEXHAUSTEDにします。
...
| 同じfailureが修正後も継続 | 自動。エラー種別・ファイル・約10行単位の位置で比較 |
| 編集3回で進展なし | 自動。明示的なedit/write/patchを集計 |
...
問題キーはセッション内の最初の失敗検証を基準に固定する保守的なv1です。
コマンド名の変更による相談予算の迂回を防ぐ一方、独立した複数バグも同じ予算を共有します。
検出器は一般的な出力のヒューリスティックであり、誤検知し得ます。
...
停止中の迂回拒否、契約した1操作、重複相談、2回制限、永続化とsubagent分離を検証します。
実モデルによる診断品質の評価は別途必要です。

`gal_status` のmetricsにはtool_calls、triggers、progress、gal_invocations、
advisor_accept/reject、advisor_exhausted、recovery_observationsを記録します。
